...
export function Dashboard() {
  ...
  // Real API queries with 30s polling
  ...
  const kpiCards = [
    { label: '配達完了', value: summary?.delivered_count ?? '--', icon: CheckCircle, bgColor: '#DCFCE7', iconColor: '#22C55E' },
    { label: '配達中', value: summary?.in_progress_count ?? '--', icon: Users, bgColor: '#EDE9FE', iconColor: '#8B5CF6' },
    { label: '未開始', value: summary?.not_started_count ?? '--', icon: Clock, bgColor: '#FEF3C7', iconColor: '#F59E0B' },
    { label: '留守止め', value: summary?.suspended_count ?? '--', icon: UserX, bgColor: '#F1F5F9', iconColor: '#9CA3AF' },
  ];

  // Deliverer rows from today data
  const delivererRows: any[] = todayData?.deliverers ?? [];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-[var(--border-default)]">
          ...
          <div className="space-y-4">
            {(summary?.comparison ?? [
              { label: '今日', value: summary?.completion_rate ?? 0, color: '#22C55E' },
              { label: '昨日', value: summary?.yesterday_rate ?? 0, color: '#94A3B8' },
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-[var(--border-default)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {delivererRows.map((deliverer: any) => (
                <div 
                  key={deliverer.name}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-[var(--border-default)]">
            ...
            <div className="space-y-3">
              {(todayData?.changes ?? [
                { type: 'new', label: '新規挿入', count: '--', color: '#F59E0B' },
                { type: 'suspend', label: '新規留守止め', count: '--', color: '#EF4444' },
                { type: 'resume', label: '留守止め解除', count: '--', color: '#22C55E' },
              ]).map((change: any) => (
                <div
                  key={change.type}
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: change.color }} />
                    <span className="text-sm text-[var(--text-primary)]">{change.label}</span>
                  </div>
                  <span className="text-lg font-bold text-[var(--text-primary)]">{change.count}件</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-[var(--border-default)]">
            ...
            {alertsData.length === 0 ? (
              <p className="text-sm text-center py-4" style={{ color: 'var(--text-muted)' }}>アラートはありません</p>
            ) : (
            <div className="space-y-3">
              {alertsData.map((alert: any, index: number) => (
                <div key={index} className="flex items-start gap-3 p-3 bg-red-50 rounded-lg border border-red-200 hover:bg-red-100 transition-colors cursor-pointer">
                  <div className="w-2 h-2 rounded-full mt-1.5 shrink-0" style={{ backgroundColor: alert.level === 'danger' ? 'var(--color-danger-500)' : 'var(--color-warning-500)' }} />
                  <p className="text-sm text-[var(--text-primary)] flex-1">{alert.message}</p>
                </div>
              ))}
            </div>
            )}
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-[var(--border-default)]">
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto">
              {(todayData?.recent_activities ?? []).map((activity: any, index: number) => (
                <div key={index} className="flex items-start gap-3 pb-3 border-b border-[var(--border-default)] last:border-0">
                  <div className="text-xs text-[var(--text-secondary)] font-medium w-12 pt-1">
                    {new Date(activity.time ?? activity.created_at).toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 rounded-full mt-1.5" style={{ backgroundColor: activity.color ?? 'var(--color-primary-500)' }} />
                      <p className="text-sm text-[var(--text-primary)]">{activity.message}</p>
                    </div>
                  </div>
                </div>
              ))}
              {(!todayData?.recent_activities || todayData.recent_activities.length === 0) && (
                <p className="text-sm text-center py-4" style={{ color: 'var(--text-muted)' }}>アクティビティなし</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-[var(--border-default)]">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="px-2 py-1 bg-orange-100 text-orange-700 text-xs font-bold rounded">
                {(todayData?.issue_reports ?? []).filter((r: any) => r.status === 'pending').length} 未解決
              </span>
            </div>
            <div className="space-y-3">
              {(todayData?.issue_reports ?? []).map((report: any) => (
                <div 
                  key={report.id}
